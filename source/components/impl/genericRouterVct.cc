...
#ifndef  _genericRouterVct_cc_INC
#define  _genericRouterVct_cc_INC

#include	"genericRouterVct.h"
using namespace std;

GenericRouterVct::GenericRouterVct ()
{
    name = "Router" ;
    ticking = false;
}  /* -----  end of method GenericRouterVct::GenericRouterVct  (constructor)  ----- */
...
void
GenericRouterVct::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
GenericRouterVct::set_grid_x_location( uint port, uint x_node, uint value)
{
    decoders[port].grid_xloc[x_node]= value;
}

void
GenericRouterVct::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
GenericRouterVct::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        ...
        case TICK_EVENT:
            ...
            break;
        default:
            _DBG("GenericRouterVct:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function GenericRouterVct::process_event  ----- */

string
GenericRouterVct::print_stats()
{
    stringstream str;
...
void
GenericRouterVct::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    if(data->type == FLIT_ID)
    {
        ...
        if( data->ptr->type == TAIL || 
            (data->ptr->type == HEAD && static_cast<HeadFlit*>(data->ptr)->msg_class == ONE_FLIT_REQ) )
        {
            packets++;
        }
    ...
    else
    {
        ...
        delete e;
    return ;
}		/* -----  end of function GenericRouterVct::handle_link_arrival_event  ----- */

void
GenericRouterVct::do_switch_traversal()
{
    /* Switch traversal */
...
//                    if(!send_early_credit)
                ...
                else
                {
                    ...
                    {
#ifdef _DEBUG_ROUTER
                        _DBG(" Waiting for flits on inch %d. Credits are %d",
                             iport,downstream_credits[oport][och]);
#endif
                    }
...
void
GenericRouterVct::do_switch_allocation()
{
    /* Switch Allocation */
    for( uint i=0; i<ports*vcs; i++)
...
void
GenericRouterVct::handle_tick_event ( IrisEvent* e )
{
    ...
    for( uint i=0; i<(ports*vcs); i++)
    {
        if( input_buffer_state[i].pipe_stage == IB )
        {
            ...
            if( !swa.is_requested(oport,iport) 
                && downstream_credits[oport][och] == credits)
            {
            ...
            else
            {
                ...
                else if(downstream_credits[input_buffer_state[i].possible_oports[0]][input_buffer_state[i].possible_ovcs[0]] != credits)
                {
...
}		/* -----  end of function GenericRouterVct::handle_input_arbitration_event  ----- */

string
GenericRouterVct::toString() const
{
    stringstream str;
    str << "GenericRouterVct"
        << "\t addr: " << address
        << " node_ip: " << node_ip
...
void
GenericRouterVct::send_credit_back(uint i)
{
...
#endif   /* ----- #ifndef _genericRouterVct_cc_INC  ----- */
